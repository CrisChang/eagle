...
public class CookieAutoLoginFilter extends AbstractAutoLoginFilter implements Filter {
	private static final  Log LOG = LogFactory.getLog(CookieAutoLoginFilter.class);
	
	private UserSecurityBean userSecurityBean;
	private final static String USER_SECURITY_BEAN_ON_COOKIE = "userSecurityBeanOnCookie";
    ...
    protected void doInit(FilterConfig filterConfig) throws ServletException {
        ...
        this.userSecurityBean = (UserSecurityBeanOnCookie) WebHttpSpringUtils.getBean(CookieAutoLoginFilter.USER_SECURITY_BEAN_ON_COOKIE, this.getServletContext());
        if (null == this.userSecurityBean){
			throw new IllegalArgumentException(String.format(
					"CookieAutoLoginFilter has to need the bean of userSecurityBeanOnCookie!"));
        }
