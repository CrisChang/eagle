...
import com.keel.utils.ahocorasick.interval.IntervalTree;
import com.keel.utils.ahocorasick.interval.Intervalable;
...
public class Trie
{
     ...
     * 屏蔽词列表
     * */
    private Map<Character, Character> invalidCharacters = new HashMap<Character, Character>() {
        {  
        put('*', ' ');  
        put('#', ' ');  
        }              
    ...
    public Collection<Emit> parseText(String text)
    {
        ...
        for (Character character : text.toCharArray())
        {
        	if(validCharacter(character)){
	            ...
	            if(storeEmits(position, currentState, collectedEmits, invalidCharacterLength)){
	            	invalidCharacterLength = 0;
	            }
        	} else {
    ...
    private boolean validCharacter(char ch){
    	if(this.invalidCharacters.containsKey(ch)){
    		return false;
    	} else {
    		return true;
    	}
     ...
     */
    private static boolean storeEmits(int position, State currentState, List<Emit> collectedEmits, int invalidCharacterLength)
    {
        ...
        if (emits != null && !emits.isEmpty())
        {
            ...
            {
                collectedEmits.add(new Emit(position - emit.length() + 1 - invalidCharacterLength, position, emit));
            }
